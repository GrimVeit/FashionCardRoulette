...
public class AllSameColorAndEven_TaskCondition : ITaskCondition
{
    ...
    public int ID { get; }
    public int NeedCountNumber { get; } = 5; // или сколько чисел используем
    public event Action<List<int>> OnTaskConditionMet_CellIndexes;

    public AllSameColorAndEven_TaskCondition(TaskType taskType, int id, int claimCoins)
    {
        TaskType = taskType;
        ID = id;
        ClaimCoins = claimCoins;
        TaskSmallDescription = "all numbers same color and even";
        TaskFullDescription = "all numbers must be even and all the same color (all red or all black)";
    }
    ...
    public bool IsMet(Dictionary<int, NumberValue> usedCells)
    {
        ...
        {
            if (cell.Number % 2 != 0) // проверяем на чётность
                return false;
