...
public class AllBlack_SumAtLeast40_TaskCondition : ITaskCondition
{
    ...
    public AllBlack_SumAtLeast40_TaskCondition(TaskType taskType, int id, int minSum, int claimCoins)
    {
        TaskType = taskType;
        ID = id;
        _minSum = minSum;

        TaskSmallDescription = $"all black, sum ≥ {_minSum}";
        TaskFullDescription = $"all numbers must be black and sum at least {_minSum}";
    ...
    public bool IsMet(Dictionary<int, NumberValue> usedCells)
    {
        ...
        {
            if (kvp.Value.Color != ColorNumber.Black)
                return false;
